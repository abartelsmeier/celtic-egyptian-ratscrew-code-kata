...
namespace CelticEgyptianRatscrewKata.PenaltyGame
{
    ...
    /// </summary>
    public class PenaltyGameState : IPenaltyGameState
    {
        ...
        /// </summary>
        public PenaltyGameState(Cards stack, IDictionary<string, Cards> decks)
        {
            _stack = stack;
            _decks = decks;
        }
        ...
        public Card PlayCardOutOfTurn(string playerId)
        {
            if (!_decks.ContainsKey(playerId)) throw new ArgumentException("The selected player doesn't exist");
            if (!_decks[playerId].Any()) throw new ArgumentException("The selected player doesn't have any cards left");
